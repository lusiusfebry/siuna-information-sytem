...
export const initScheduler = () => {
    ...
    logger.info(`Audit Log Cleanup scheduled at: ${CRON_SCHEDULE}, Retention: ${RETENTION_DAYS} days`);
    logger.info(`Asset Reminders scheduled at: ${ASSET_REMINDER_CRON}`);

    cron.schedule(CRON_SCHEDULE, async () => {
        logger.info('Running daily audit log cleanup...');
        try {
            const count = await auditService.cleanupOldLogs(RETENTION_DAYS);
            logger.info(`Audit log cleanup completed. Deleted ${count} old records.`);
        } catch (error) {
    ...
    cron.schedule(ASSET_REMINDER_CRON, async () => {
        logger.info('Running asset reminder scan...');
        try {
            await notificationService.checkAssetRemindersAndNotify();
        } catch (error) {
            logger.error('Asset reminder scan failed:', error);
        }
    });
};
