...
router.put('/company-settings', authenticate, checkPermission(RESOURCES.COMPANY_SETTINGS, ACTIONS.UPDATE), (req, res, next) => companySettingsController.updateSettings(req, res, next));
router.post('/company-settings/logo', authenticate, checkPermission(RESOURCES.COMPANY_SETTINGS, ACTIONS.UPDATE), uploadCompanyLogo, (req, res, next) => companySettingsController.uploadLogo(req, res, next));
