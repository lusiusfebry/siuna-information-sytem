...
class FacilityMasterDataService {
    ...
    async findAll(model: ModelStatic<Model>) {
        ...
        const cacheKey = `facility_master_data:${modelName}:all`;
    ...
    async invalidateCache(modelName: string) {
        ...
        await cacheService.delPattern(`facility_master_data:${modelName}:*`);
        await cacheService.delPattern(`cache:/api/facility/master/${slug}*`);
    }
