...
import { FacMaintenanceCategory } from '../../../types/facility';
...
const MaintenanceCategoryPage = () => {
    ...
    const [selectedItem, setSelectedItem] = useState<FacMaintenanceCategory | null>(null);
    ...
    const { data, isLoading } = useFacMaintenanceCategoryList({ page, limit: 10, search, status });
    const createMutation = useCreateFacilityMasterData<FacMaintenanceCategory>('maintenance-category');
    const updateMutation = useUpdateFacilityMasterData<FacMaintenanceCategory>('maintenance-category');
    const deleteMutation = useDeleteFacilityMasterData('maintenance-category');

    const columns: Column<FacMaintenanceCategory>[] = [
    ...
    const formFields = [
        { name: 'nama', label: 'Nama Kategori', type: 'text' as const, required: true, placeholder: 'Contoh: Listrik, Plumbing, AC', autoTitleCase: true },
    ...
    const handleEdit = (item: FacMaintenanceCategory) => { setModalMode('edit'); setSelectedItem(item); setIsModalOpen(true); };
    const handleDelete = (item: FacMaintenanceCategory) => { setSelectedItem(item); setIsConfirmOpen(true); };

    const onFormSubmit = (formData: Record<string, unknown>) => {
        if (modalMode === 'create') {
            createMutation.mutate(formData as unknown as Partial<FacMaintenanceCategory>, {
        ...
        <MasterDataLayout view={layout}>
            <div className="space-y-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Kategori Maintenance</h1>
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola kategori pemeliharaan fasilitas</p>
                    </div>
                </div>
                ...
                <ConfirmDialog isOpen={isConfirmOpen} title="Hapus Kategori"
