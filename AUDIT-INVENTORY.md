...
- [x] **B-3** **[INV-M03] — FIXED.** Lokasi unit pasca "Ke Gedung/Mess" kini **ditampilkan** di hasil lookup QR. `lookupQR` (label.service) membaca placement aktif dari `facility_assets` (`getActiveFacilityPlacement` → room + building) dan menyertakannya sebagai `facility_placement` pada hasil serial & asset-tag; `LabelPage` menampilkan blok "Lokasi Terpasang (Gedung/Mess)". **Keputusan desain:** lokasi TIDAK didenormalisasi ke kolom baru di `inv_serial_number` (berisiko drift); sumber otoritatif tetap baris `facility_assets` aktif yang sudah ditulis siklus transaksi (C01). Diverifikasi: `tsc` backend + frontend bersih.
- [x] **B-4** **[INV-M04] — FIXED.** Retur/tarik via inventory **menutup** baris `facility_assets` (`closeFacilityPlacement`, status → `Ditarik`) lewat siklus transaksi (bagian dari perbaikan C01), sehingga placement tak lagi basi saat unit kembali ke gudang. **Sisa (paranoid/soft-delete) ditutup by-design:** `facility_assets` adalah tabel ledger penempatan — sekelas `facility_occupants`/`facility_work_orders`/`inv_transaksi` yang juga non-paranoid sesuai konvensi proyek (hanya master-data yang paranoid). Tidak ada jalur hard-delete (tak ada endpoint/`.destroy`); baris hanya ditutup dengan `status`+`tanggal_penarikan`+`transaksi_id` sehingga jejak sudah auditable. Menambah soft-delete tak memberi manfaat dan melanggar konvensi.
...
| **INV-M03** | Lokasi serial pasca Ke-Gedung/Mess tak tersimpan di record serial | B-3 | Minor | Desain | FIXED — lookup QR membaca placement aktif `facility_assets` (room+building) & tampilkan "Lokasi Terpasang"; tanpa denormalisasi kolom |
| **INV-M04** | facility_assets non-paranoid & tak terikat siklus transaksi → placement basi | B-4 | Minor | Integritas | FIXED — placement ditutup oleh siklus transaksi (C01); non-paranoid ditutup by-design (tabel ledger, tak ada hard-delete) |
